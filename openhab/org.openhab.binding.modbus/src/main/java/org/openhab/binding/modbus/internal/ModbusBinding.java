...
public class ModbusBinding extends AbstractBinding<ModbusBindingProvider> {
	...
	protected void internalReceiveCommand(String itemName, Command command) {
		for (ModbusBindingProvider provider : providers) {
			if (provider.providesBindingFor(itemName)) {
				...
				slave.executeCommand(command, config.readRegister, config.writeRegister, config.readRegisterBitAddress, config.writeRegisterBitAddress);
			}
		}
	}
	...
	protected void internalUpdateItem(String slaveName, BitVector coils,
		...
		for (ModbusBindingProvider provider : providers) {
			if (provider.providesBindingFor(itemName)) {
				ModbusBindingConfig config = provider.getConfig(itemName);
				if (config.slaveName.equals(slaveName)) {
					boolean state = coils.getBit(getBitNumber(config.readRegister, config.readRegisterBitAddress));
	...
	public static int getBitNumber(int register, int bitNumberInRegister) {
		return bitNumberInRegister + (16 * register);
	}
