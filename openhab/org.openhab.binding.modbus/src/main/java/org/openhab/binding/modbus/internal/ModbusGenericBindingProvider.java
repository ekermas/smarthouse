...
public class ModbusGenericBindingProvider extends AbstractGenericBindingProvider implements ModbusBindingProvider {
	...
	public class ModbusBindingConfig implements BindingConfig {
		...
		int readRegisterBitAddress;
		int writeRegisterBitAddress;
		...
		ModbusBindingConfig(Item item, String config) throws BindingConfigParseException {
			...
			try {
				...
				slaveName = items[0];
				if (items.length == 2) {
					setReadRegister(items[1]);
					setWriteRegister(items[1]);
		...
		private void setReadRegister(String item) {
			String [] items = item.split("#");
			readRegister = Integer.valueOf(items[0]);
			readRegisterBitAddress = Integer.valueOf(items[1]);
		}
		
		private void setWriteRegister(String item) {
			String [] items = item.split("#");
			writeRegister = Integer.valueOf(items[0]);
			writeRegisterBitAddress = Integer.valueOf(items[1]);
		}
		...
		private void assignRegisters(String item)
			...
			if (item.startsWith("<")) {
				setReadRegister(item.substring(1, item.length()));
			} else if (item.startsWith(">")) {
				setWriteRegister(item.substring(1, item.length()));
			} else {
